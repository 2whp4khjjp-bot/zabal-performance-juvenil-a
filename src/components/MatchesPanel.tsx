...
export function MatchesPanel({ players, matches, saving, onSave, onUpdate, onDelete }: Props) {
  ...
  const rankings = useMemo(() => ({
    scorers: [...totals].filter((item) => item.goals > 0).sort((a, b) => b.goals - a.goals || b.minutes - a.minutes).slice(0, 3),
    callUps: [...totals].filter((item) => item.callUps > 0).sort((a, b) => b.callUps - a.callUps || b.minutes - a.minutes).slice(0, 20),
  ...
  return (
    <main className="page-shell matches-page">
      ...
      {mode === 'new' ? <form className="match-form" onSubmit={(event) => void submit(event)} noValidate>
        ...
        <article className="panel-card ranking-card ranking-card--scorers"><div className="ranking-heading"><span><Goal /></span><div><p className="eyebrow eyebrow--dark">Top 3</p><h2>Máximos goleadores</h2></div></div><RankingList items={rankings.scorers} value={(item) => item.goals} suffix="goles" podium /></article>
...
function RankingList({ items, value, suffix, podium = false }: { items: TotalRow[]; value: (item: TotalRow) => number; suffix: string; podium?: boolean }) {
  ...
  return <ol className="ranking-list">{items.map((item, index) => <li key={item.player.id} className={podium && index < 3 ? `podium-${index + 1}` : ''}><span className="ranking-position">{index + 1}</span><span className="ranking-player"><strong>{item.player.name}</strong><small>Dorsal {item.player.number ?? '—'}</small></span><strong className="ranking-value">{value(item)} <small>{suffix}</small></strong></li>)}</ol>;
}
