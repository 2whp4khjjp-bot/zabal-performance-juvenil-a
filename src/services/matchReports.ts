...
export const generateMinutesPdf = async (players: Player[], matches: MatchRecord[]) => {
  ...
  const rows = [...totals.values()].sort((a, b) => (a.player.number ?? 999) - (b.player.number ?? 999) || a.player.order - b.player.order);
  ...
  autoTable(doc, {
    startY: 64,
    head: [['Dorsal', 'Jugador', 'Conv.', 'Tit.', 'PJ', 'Minutos', 'Media/PJ']],
    body: rows.map((row) => [
      row.player.number ?? '—', row.player.name, row.callUps, row.starts, row.appearances, row.minutes,
    ...
    columnStyles: {
      0: { cellWidth: 16, halign: 'center' }, 1: { cellWidth: 55 }, 2: { cellWidth: 18, halign: 'center' },
      3: { cellWidth: 16, halign: 'center' }, 4: { cellWidth: 16, halign: 'center' },
      5: { cellWidth: 24, halign: 'right', fontStyle: 'bold' }, 6: { cellWidth: 24, halign: 'right' },
    },
  });
  addPdfFooters(doc);
