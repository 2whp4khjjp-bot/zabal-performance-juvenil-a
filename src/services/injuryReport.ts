...
export async function createTeamInjuriesPdf(players: Player[]) {
  ...
  autoTable(doc, {
    startY: 85,
    head: [['Dorsal', 'Jugador', 'Estado actual', 'Periodos', 'Días', 'Motivo actual']],
    ...
    columnStyles: { 0: { cellWidth: 14 }, 1: { cellWidth: 43 }, 2: { cellWidth: 27 }, 3: { cellWidth: 17 }, 4: { cellWidth: 14 } },
    didParseCell: (data) => {
      if (data.section === 'body' && data.column.index === 2 && data.cell.raw === 'BAJA ACTIVA') {
  ...
  if (report.history.length) {
    autoTable(doc, {
      startY: 33,
      head: [['Dorsal', 'Jugador', 'Inicio', 'Alta', 'Días', 'Motivo']],
      ...
      headStyles: { fillColor: [22, 54, 95], textColor: [255, 255, 255] },
      alternateRowStyles: { fillColor: [245, 248, 251] },
      columnStyles: { 0: { cellWidth: 14 }, 1: { cellWidth: 40 }, 2: { cellWidth: 23 }, 3: { cellWidth: 23 }, 4: { cellWidth: 14 } },
      didParseCell: (data) => {
        if (data.section === 'body' && data.column.index === 3 && data.cell.raw === 'ACTIVA') {
          data.cell.styles.textColor = [174, 46, 59];
          data.cell.styles.fontStyle = 'bold';
        }
      },
